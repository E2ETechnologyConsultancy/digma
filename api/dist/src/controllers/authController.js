...
exports.logout = exports.changePassword = exports.getProfile = exports.login = exports.register = void 0;
...
const register = async (req, res) => {
    try {
        ...
        if (!userTenantId) {
            ...
            const defaultTenant = await Tenant_1.default.findOne({ name: 'default-tenant' });
            if (defaultTenant) {
                userTenantId = defaultTenant._id;
            }
            else {
                // Create default tenant if it doesn't exist
                const newTenant = await Tenant_1.default.create({
                    name: 'default-tenant',
        ...
        const tenant = await Tenant_1.default.findById(userTenantId);
        if (!tenant) {
...
const login = async (req, res) => {
    try {
        ...
        // Find user
        const user = await User_1.default.findOne({ email: email.toLowerCase() }).populate('tenant', 'name');
        if (!user) {
            res.status(401).json({
